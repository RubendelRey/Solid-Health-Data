...
import bulkExportService from "../../api/bulkExportService";
...
const BulkDataExport = () => {
	...
	const [loadTestForm] = Form.useForm();
	...
	const handleStartLoadTesting = async () => {
		if (!solidConnected) {
		...
		try {
			...
			let pathValues = {};
			try {
				pathValues = await pathForm.validateFields();
			} catch (error) {
				console.warn("Path form validation failed, using defaults:", error);
				pathValues = {
					baseFolder: "load-tests",
					subfolder: "dental-clinic",
					filePrefix: "test",
				};
			}
					...
					<div>
						...
						<ul>
							...
							<li>
								...
								{pathValues.subfolder || "dental-clinic"}
							</li>
						</ul>
					</div>
				),
					...
					try {
						const response = await bulkExportService.exportAllPatientsToSolid({
							...
							pathConfiguration: {
								...
								subfolder: pathValues.subfolder || "dental-clinic",
						...
						if (response.stats.failed === 0) {
							message.success(
								`Load testing completed successfully! Generated ${response.stats.totalTriples.toLocaleString()} triples in ${
									response.stats.completed
								} test(s).`
							);
						} else {
							message.warning(
								`Load testing completed with ${response.stats.completed} successes and ${response.stats.failed} failures`
							);
						}
					} catch (error) {
						console.error("Error during load testing:", error);
						message.error("Load testing failed: " + error.message);
					} finally {
						setExporting(false);
					}
				},
			...
			console.error("Error in handleStartLoadTesting:", error);
			message.error("Failed to start load testing: " + error.message);
		}
	};
		...
		<div style={{ padding: 24 }}>
			...
			<Row gutter={[24, 24]}>
						...
						{checkingAuth ? (
							...
							<>
								<Alert
									...
									description="You need to connect to your Solid pod to run load testing. Enter your Solid provider URL below."
									type="info"
									showIcon
				...
				<Col xs={24} lg={12}>
					...
					>
						<Alert
							message="Solid Performance Testing"
							description="Configure the number of tests and RDF triples per test to evaluate Solid pod performance."
							type="info"
							showIcon
											...
											<Row gutter={16}>
												...
												<Col span={8}>
													<Form.Item
														name="subfolder"
														label="Subfolder"
														initialValue="dental-clinic"
													>
														<Input placeholder="dental-clinic" />
													</Form.Item>
												</Col>
												<Col span={8}>
													<Form.Item
						...
						<Button
							...
							onClick={handleStartLoadTesting}
							loading={exporting}
				...
				<Col xs={24} lg={12}>
					...
					>
						<Space direction="vertical" style={{ width: "100%" }}>
							<Alert
								message="What is Load Testing?"
								description="Load testing measures how well your Solid pod handles multiple data operations by uploading specified quantities of RDF triples."
								type="info"
								showIcon
							/>

							<Alert
								message="RDF Triple Format"
								description="Tests use N-Triples format with dental healthcare vocabulary to simulate realistic data upload scenarios."
								type="info"
								showIcon
							/>
							...
							<Alert
								...
								description="Monitor upload speed, response times, and throughput to evaluate your Solid pod's performance characteristics."
								type="success"
								showIcon
							/>
						</Space>
			...
			{exporting && (
				<Card title="Load Testing Progress" style={{ marginTop: 24 }}>
			...
			{exportStats && (
				<Card title="Load Testing Statistics" style={{ marginTop: 24 }}>
					<Row gutter={16}>
						<Col span={4}>
							<Statistic
								title="Total Tests"
								value={exportStats.totalTests}
						...
						<Col span={4}>
							<Statistic
								title="Total Triples"
								value={exportStats.totalTriples}
						...
						<Col span={4}>
							<Statistic
								title="Triples/Second"
								value={exportStats.triplesPerSecond}
								precision={2}
								prefix={<SyncOutlined />}
			...
			{exportResults.length > 0 && (
				<Card title="Load Testing Results" style={{ marginTop: 24 }}>
					<Tabs
						activeKey={activeTab}
