...
class BaseRepository {
	...
	async initialize() {
		try {
			...
			this.db = this.client.db(process.env.MONGO_DB_NAME || "hospital");
	...
	async createMany(dataArray) {
		try {
			...
			console.error(`Error in createMany for ${this.collectionName}:`, error);
			throw error;
		}
	}
	...
	async deleteMany(query = {}) {
		try {
			...
			console.error(`Error in deleteMany for ${this.collectionName}:`, error);
			throw error;
		}
	}
	...
	async countDocuments(query = {}) {
		try {
			...
			console.error(
				`Error in countDocuments for ${this.collectionName}:`,
				error
			);
			throw error;
		}
